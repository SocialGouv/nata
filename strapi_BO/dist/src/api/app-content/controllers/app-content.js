...
exports.default = {
    getContent: async (ctx, next) => {
        const data = {};
        const allApiServices = Object.keys(await strapi.services).filter((service) => service.startsWith("api::") &&
            service !== "api::app-content.app-content");
        const saveData = async (serviceKey, service) => {
            const populating = serviceKey === "question"
                ? { responses: true }
                : serviceKey === "response"
        ...
        await Promise.all(allApiServices.map((service) => {
